...
def collate_fn(batch):
    ...
    imgs = paddle.stack(imgs, 0)
    max_box_len = max([l.shape[0] for l in labels])
    # labels = [torch.from_numpy(l) for l in labels]
    # filled_labels = torch.zeros(batch_size, max_box_len, 6)
    # labels_len = torch.zeros(batch_size)
    labels = [paddle.to_tensor(l) for l in labels]
    filled_labels = paddle.zeros([batch_size, max_box_len, 6])
    labels_len = paddle.zeros([batch_size])
    ...
    def __getitem__(self, files_index):
        ...
        for k in range(num_objs):
            ...
            if h > 0 and w > 0:
                ...
                ids[k] = label[1]
                bbox_xys[k] = bbox_xy
        ret = {'input': imgs.numpy(), 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh, 'reg': reg, 'ids': ids, 'bbox': bbox_xys}
        return ret
        # return imgs, hm, reg_mask, ind, wh, reg, ids, bbox_xys
