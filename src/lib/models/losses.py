...
def _neg_loss(pred, gt):
  ...
  neg_inds = gt.less_than(paddle.ones(gt.shape, dtype=gt.dtype)).cast('float32')
...
def _reg_loss(regr, gt_regr, mask):
  ...
  num = mask.cast('float32').sum()
  mask = mask.unsqueeze(2).expand_as(gt_regr).cast('float32')
  regr = regr * mask
  gt_regr = gt_regr * mask
...
class RegL1Loss(nn.Layer):
  ...
  def forward(self, output, mask, ind, target):
    ...
    mask = mask.unsqueeze(2).cast('float32').expand_as(pred)
...
class RegWeightedL1Loss(nn.Layer):
  def __init__(self):
...
def compute_bin_loss(output, target, mask):
    ...
    output = output * mask.cast('float32')
...
def compute_rot_loss(output, target_bin, target_res, mask):
    ...
    if target_bin[:, 0].nonzero().shape[0] > 0:
        ...
        valid_target_res1 = paddle.index_select(target_res, idx1.cast('int32'), 0)
    ...
    if target_bin[:, 1].nonzero().shape[0] > 0:
        ...
        valid_target_res2 = paddle.index_select(target_res, idx2.cast('int32'), 0)
